...
@Cmd('backOldRoute')
export class BackOldRouteCmd extends BaseCmd {
  getOldTransDef; setTransCode; setOldTransDef;
  ...
  execute(data: {refGetSetFuncs: any, refReadonlyProps: any, params?: any}): void {
    this.getOldTransDef = data.refGetSetFuncs.getOldTransDef;
    ...
    // 初始化成员变量
    const oldDef = this.getOldTransDef();
    this.setTransCode(oldDef.transCode);
    this.setOldTransDef(undefined);
    const sub = new Subject();
    this.setTransactionContextChangeSub(sub);
    this.setTransactionContextChangeOb(sub.asObservable());
    // 加载交易
    this.loadTrans(oldDef.transCode);
  }
